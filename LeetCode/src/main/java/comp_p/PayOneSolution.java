...
class RunCollection {
    ...
    public int personalBest() {
        ...
        return runs.stream().filter(o->o.obstacleTimes.size()==course.obstacleCount).mapToInt(num->num.getRunTime()).min().orElse(Integer.MAX_VALUE);
    }

    public int bestOfBests(){
        int total = 0;
        for(int index = 0; index < course.obstacleCount; index++){
            int subtotal = Integer.MAX_VALUE;
            for (Run run : runs) {
                if (run.obstacleTimes.size() > index) {
                    int num = run.obstacleTimes.get(index);
                    subtotal = Math.min(subtotal, num);
                }
            }
            total+=subtotal;
    ...
    public int bestOfBests2(){
        return IntStream.range(0,course.obstacleCount)
                .map(index->
                        runs.stream().filter(run -> run.obstacleTimes.size() > index)
                                .mapToInt(run-> run.obstacleTimes.get(index))
                                .min()
                                .orElse(0)
                ).sum();
    }
