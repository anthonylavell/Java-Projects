...
public class BankAccount {
    ...
    public void textData(String path){
        try {
            File file = new File(path);
            Scanner scanner = new Scanner(file);
            List<String> lines = scanner.useDelimiter("\n")
                    .tokens()
                    .filter(token -> !token.isBlank() && !token.startsWith("ts_iso"))
                    .toList();
            int i = 0;
            process(lines);
        }catch (Exception e){
    ...
    private void jsonData(String path){
        try {
            List<String> accountTitles = List.of("ts_iso","type","account_id","account_name",
                    "currency","amount","source_account_id","destination_account_id","note");
            String jsonText = Files.readString(Paths.get(path));
            JSONArray jsonArray = new JSONArray(jsonText);
            List<String> lines = IntStream.range(0, jsonArray.length())
                    .mapToObj(jsonArray::getJSONObject)
                            .map(jsonObject -> accountTitles.stream()
